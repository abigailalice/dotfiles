...
do
	...
	-- Show the cursorline only on the active pane
	local cursorline_group = vim.api.nvim_create_augroup("cursorline-active-win", { clear = true })
...
do
	...
	vim.lsp.enable({ "rust_analyzer", "hls", "nil_ls" })
end
...
do
	...
	vim.pack.add({ { src = gh("nvim-treesitter/nvim-treesitter"), version = "master" } })
...
-- Set leader before anything else (better in init.lua, but here for completeness)
